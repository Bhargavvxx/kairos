...
async def extract_text_from_file(file: UploadFile, use_streaming: bool = False) -> Tuple[str, Dict[str, Any]]:
    ...
    try:
        ...
        if file.content_type == 'application/pdf':
            ...
            text = await loop.run_in_executor(executor, extract_pdf_text_sync, contents, metadata)
            ...
            loop = asyncio.get_running_loop()
            text = await loop.run_in_executor(executor, extract_docx_text_sync, contents, metadata)
...
def extract_pdf_text_sync(contents: bytes, metadata: Dict[str, Any]) -> str:
...
def extract_docx_text_sync(contents: bytes, metadata: Dict[str, Any]) -> str:
...
async def extract_text_streaming(file: UploadFile, metadata: Dict[str, Any]) -> Tuple[str, Dict[str, Any]]:
    ...
    temp_path = os.path.join(tempfile.gettempdir(), f"kairos_upload_{uuid.uuid4()}")
...
async def process_document_async(
    ...
    try:
        async with status_lock:
            ...
            processing_status[tracking_id]["started_at"] = datetime.now(timezone.utc).isoformat()
        ...
        try:
            ...
            # Update status and re-raise as a regular exception (not HTTPException)
            # since this runs in a background task with no HTTP context
            raise RuntimeError(f"Vector processing failed: {str(e)}")
        ...
        async with status_lock:
            ...
            processing_status[tracking_id]["completed_at"] = datetime.now(timezone.utc).isoformat()
...
async def get_processing_status(tracking_id: str):
    ...
    async with status_lock:
        ...
        doc_status = processing_status[tracking_id].copy()
    
    # Add time estimates for in-progress documents
    if doc_status["status"] == "processing" and "started_at" in doc_status:
        started = datetime.fromisoformat(doc_status["started_at"])
        elapsed = (datetime.now(timezone.utc) - started).total_seconds()
        
        # Estimate based on progress
        if doc_status["progress"] > 0:
            estimated_total = elapsed / (doc_status["progress"] / 100)
            doc_status["estimated_completion"] = max(0, estimated_total - elapsed)
        else:
            doc_status["estimated_completion"] = 30  # Default estimate
    
    return ProcessingStatusResponse(**doc_status)
...
async def stream_processing_status(tracking_id: str, updates_per_sec: int = 2):
    ...
    async def event_generator():
        ...
        while True:
            async with status_lock:
                doc_status = processing_status.get(tracking_id)
            ...
            if doc_status:
                ...
                current_update = {
                    ...
                    "stages": doc_status["stages"]
                }
                
                if current_update != last_update:
                    yield {
                        "event": "status",
                        "data": json.dumps(doc_status)
                    }
                    last_update = current_update
                
                # End stream when processing is complete
                if doc_status["status"] in ["completed", "failed"]:
                    yield {
                        "event": "complete",
