...
class RateLimiter:
    ...
    async def acquire(self):
        """Acquire both semaphore slot and rate limit permission.
        The semaphore is held by the caller (via __aenter__/__aexit__).
        """
    ...
    def release(self):
        """Release the semaphore slot after the LLM call completes."""
...
async def run_hackrx_submission(
    ...
    """
    Optimized endpoint for batch document processing.
    """
...
# Insurance/Finance domain terms for priority matching (specialization layer)
_INSURANCE_FINANCE_KEY_TERMS = [
    # Policy & Payment
    ...
    "maternity benefit", "newborn cover", "AYUSH treatment", "cashless network", "network hospitals",
    # Conditions
    "pre-existing disease", "waiting period", "specific waiting period", "chronic illness",
    "critical illness", "cumulative bonus", "no claim bonus",
    # Exclusions
    "permanent exclusions", "standard exclusions",
    # Claims
    "claim settlement", "cashless claim", "reimbursement claim", "claim intimation",
    "claim rejection", "TPA", "grievance",
    # Finance
    "interest rate", "fixed deposit", "mutual fund", "credit score", "balance sheet",
    "profit and loss", "cash flow", "capital gain", "tax benefit", "net worth",
    "policyholder", "insured person", "beneficiary", "IRDAI", "SEBI", "RBI",
]

def extract_key_terms(question: str) -> List[str]:
    """Extract key terms from question — General Core + Insurance/Finance specialization."""
    import re
    keywords = []
    question_lower = question.lower()
    
    # === LAYER 1: Insurance/Finance domain term matching (priority) ===
    for term in _INSURANCE_FINANCE_KEY_TERMS:
        if term.lower() in question_lower:
            keywords.append(term)
    
    # === LAYER 2: General NLP-based extraction ===
    stop_words = {'the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been',
                  'has', 'have', 'had', 'do', 'does', 'did', 'will', 'would',
                  'could', 'should', 'may', 'might', 'can', 'shall', 'of', 'in',
                  'to', 'for', 'with', 'on', 'at', 'by', 'from', 'it', 'this',
                  'that', 'what', 'which', 'who', 'how', 'when', 'where', 'why',
                  'and', 'or', 'but', 'not', 'if', 'so', 'as', 'about', 'than',
                  'me', 'my', 'i', 'we', 'you', 'they', 'he', 'she', 'its'}
    
    # Multi-word phrases
    phrases = re.findall(r'\b[a-z]+(?:\s+[a-z]+){1,2}\b', question_lower)
    for phrase in phrases:
        words = phrase.split()
        meaningful_words = [w for w in words if w not in stop_words]
        if len(meaningful_words) >= 1 and phrase not in keywords:
    ...
    return keywords[:7]  # Slightly increased limit for domain terms
