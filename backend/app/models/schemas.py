...
# --- ENUMs for controlled vocabularies (must be defined before SmartNodeTypeValidator) ---

class NodeType(str, Enum):
    # Universal entity types (work across all domains)
    PERSON = "Person"
    ...
    # Domain-bridging types (useful across insurance, legal, finance, tech, etc.)
    POLICY = "Policy"
    COMPANY = "Company"  # Alias for Organization, kept for compatibility
    CONTRACT = "Contract"
    ...
    # Insurance/Finance domain specialization
    PREMIUM = "Premium"
...
class SmartNodeTypeValidator:
    """
    Intelligent node type validator that maps any LLM output to valid NodeType values.
    Handles automatic type normalization: Form -> Document, Property -> Entity, etc.
    """
    ...
    TYPE_MAPPINGS = {
        ...
        # Insurance/Finance domain specialization
        'rider': 'Rider', 'endorsement': 'Endorsement', 'exclusion': 'Exclusion',
        'coverage': 'Coverage', 'deductible': 'Deductible', 'premium': 'Premium',
        'copay': 'Amount', 'coinsurance': 'Amount', 'beneficiary': 'Beneficiary',
        'policyholder': 'Policyholder', 'insurer': 'Insurer', 'insured': 'Policyholder',
        'underwriter': 'Underwriter', 'claimant': 'Person', 'nominee': 'Beneficiary',
        'sum_insured': 'Amount', 'maturity': 'Amount', 'annuity': 'Amount',
        'liability': 'Liability', 'asset': 'Asset', 'equity': 'Amount',
        'portfolio': 'Entity', 'fund': 'Entity', 'bond': 'Entity',
        'stock': 'Entity', 'dividend': 'Amount', 'interest': 'Amount',
        'loan': 'Entity', 'mortgage': 'Entity', 'credit': 'Entity',
        'debit': 'Transaction', 'transfer': 'Transaction', 'deposit': 'Transaction',
        ...
        'system': 'Technology', 'software': 'Technology', 'hardware': 'Technology',
        'database': 'Technology', 'server': 'Technology', 'api': 'Technology',
        'interface': 'Technology', 'module': 'Technology',
        'framework': 'Technology', 'library': 'Technology', 'platform': 'Technology',
        'algorithm': 'Process', 'architecture': 'Concept', 'pattern': 'Concept',
        
        # Academic specific
        'study': 'Document', 'research': 'Document', 'thesis': 'Document',
        'hypothesis': 'Concept', 'theory': 'Concept', 'experiment': 'Process',
        'finding': 'Concept', 'conclusion': 'Concept', 'methodology': 'Process',
        'author': 'Person', 'researcher': 'Person', 'professor': 'Person',
        
        # HR specific
        'employee': 'Person', 'manager': 'Person', 'department': 'Organization',
        'team': 'Organization', 'position': 'Role', 'job': 'Role',
        'leave': 'Benefit', 'bonus': 'Benefit', 'promotion': 'Event',
        'performance': 'Metric', 'kpi': 'Metric', 'target': 'Metric',
        
        # Measurement/Metrics
        'rate': 'Metric', 'ratio': 'Metric', 'score': 'Metric',
        'index': 'Metric', 'benchmark': 'Metric', 'threshold': 'Metric',
    ...
    def normalize_type(cls, node_type: str) -> str:
        """Convert any node type string to a valid NodeType enum value."""
        if not node_type:
            return "Entity"
        ...
        # Semantic analysis for unmapped types
        if any(word in normalized for word in ['money', 'dollar', 'cost', 'pay', 'price', 'fee', 'charge']):
            ...
            return "Event"
        elif any(word in normalized for word in ['technology', 'software', 'hardware', 'system', 'platform']):
            return "Technology"
        elif any(word in normalized for word in ['metric', 'kpi', 'score', 'rate', 'ratio', 'benchmark']):
            return "Metric"
        elif any(word in normalized for word in ['concept', 'idea', 'theory', 'principle', 'approach']):
            return "Concept"
        elif any(word in normalized for word in ['risk', 'threat', 'vulnerability', 'hazard', 'danger']):
            return "Risk"
        elif any(word in normalized for word in ['role', 'position', 'title', 'designation', 'function']):
            return "Role"
...
class Node(TimestampedModel, VersionedModel):
    """Represents a single entity in the knowledge graph with smart type validation."""
    id: str = Field(default_factory=lambda: f"node_{uuid.uuid4()}", description="Auto-generated unique node ID.")
    type: Union[NodeType, str] = Field(..., description="Node type - automatically normalized to valid enum")
    ...
    def normalize_node_type(cls, v):
        """Automatically normalize any node type string to a valid enum value."""
        if isinstance(v, str):
    ...
    def validate_type_is_enum(cls, v):
        """Ensure the type is a valid NodeType enum value with fallback."""
        if isinstance(v, str):
            try:
                return NodeType(v)
            except ValueError:
...
class GraphData(BaseModel):
    ...
    def validate_edge_references(cls, values):
        """Filter out edges that reference non-existent nodes instead of failing."""
        ...
        for edge in edges:
            if edge.source in node_ids and edge.target in node_ids:
                ...
                logging.getLogger(__name__).warning(
                    f'Dropping edge with invalid reference: source="{edge.source}", target="{edge.target}"'
                )
        values['edges'] = valid_edges
        return values
