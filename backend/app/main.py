...
class GeminiLLMClient:
    ...
    async def generate(self, prompt: str) -> str:
        ...
        try:
            # Detect if this is a JSON extraction prompt or a Q&A prompt
            prompt_lower = prompt.lower()
            is_extraction = (
                ("extract" in prompt_lower and ("json" in prompt_lower or "knowledge graph" in prompt_lower))
                or ("return only" in prompt_lower and "json" in prompt_lower)
...
{prompt}"""
                ...
                enhanced_prompt = f"""You are KAIROS, an intelligent document analysis assistant.
Answer questions clearly and thoroughly in natural language.
...
{prompt}"""
            ...
            # Only strip JSON fences for extraction prompts
            if is_extraction:
...
class RealLLMClient:
    ...
    async def generate(self, prompt: str) -> str:
        """Generate response from actual LLM with retry logic for rate limits."""
...
Ensure all JSON is properly formatted and valid."""
                ...
                system_prompt = """You are KAIROS, an intelligent document analysis assistant.
Answer questions clearly and thoroughly in natural language.
Use markdown formatting for readability (headings, bullet points, bold text).
Base your answers strictly on the provided context.
If the context does not contain enough information, say so honestly."""
...
app = FastAPI(
    title="KAIROS API",
    description="Backend engine for the KAIROS document intelligence platform.",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    ...
    return {
        ...
        "version": app.version,
        "status": "healthy",
        "description": "Knowledge Graph & RAG Intelligence System"
    }
...
if os.getenv("ENV", "development") == "development":
    ...
    @app.get("/debug/uploaded-docs", tags=["Debug"])
    async def get_uploaded_documents(
        vector_svc: VectorService = Depends(get_vector_service)
    ):
        """Get all uploaded documents in the system."""
        docs = vector_svc.get_documents_by_source("upload")
        return {
            "total_documents": len(docs),
            "documents": docs
        }
    
    @app.delete("/debug/clear-uploaded", tags=["Debug"])
    async def clear_uploaded_documents(
        ...
        return {
            "message": f"Cleared {deleted} uploaded documents",
