...
# === GENERAL CORE + DOMAIN SPECIALIZATION (Insurance/Finance) ===
# The system works across all document types via embeddings and BM25.
# Insurance/Finance domain terms are used as a boosting layer for improved
# relevance when processing documents in those specialized domains.

INSURANCE_FINANCE_TERMS = [
    # --- Insurance Domain ---
    # Policy & Payment
    "grace period", "free look period", "policy cancellation", "policy renewal", "premium payment",
    "policy period", "policy holder", "insured person", "beneficiary", "nominee",
    # Coverage
    ...
    "maternity benefit", "newborn cover", "organ donor expenses", "AYUSH treatment",
    "cashless treatment", "reimbursement claim", "network provider", "network hospitals",
    # Eligibility
    "entry age", "exit age", "dependent age", "age limit", "dependent coverage",
    "family floater", "individual policy", "group policy", "portability",
    # Hospitalization
    "pre-hospitalization", "post-hospitalization", "hospital cash", "day care procedures",
    "shared accommodation", "cashless network",
    # Conditions & Exclusions
    "pre-existing disease", "waiting period", "specific waiting period", "chronic illness",
    "critical illness", "permanent exclusions", "standard exclusions",
    "cumulative bonus", "automatic restoration", "no claim bonus",
    # Claims
    "claim settlement", "cashless claim", "claim intimation", "claim rejection",
    "TPA", "third party administrator", "grievance redressal",
    # Regulatory
    "IRDAI", "insurance regulator", "ombudsman", "policy wording",
    
    # --- Finance Domain ---
    # Banking & Accounts
    "savings account", "current account", "fixed deposit", "recurring deposit",
    "interest rate", "compound interest", "simple interest", "EMI",
    # Investments
    "mutual fund", "stock market", "equity", "debt fund", "SIP",
    "portfolio", "NAV", "dividend", "capital gain", "tax benefit",
    # Loans & Credit
    "loan amount", "mortgage", "collateral", "credit score", "credit limit",
    "disbursement", "prepayment", "foreclosure", "default",
    # Financial Statements
    "balance sheet", "profit and loss", "cash flow", "revenue", "expense",
    "net worth", "asset", "liability", "equity ratio", "debt ratio",
    # Compliance
    "KYC", "AML", "SEBI", "RBI", "compliance", "audit", "disclosure",
]

def is_insurance_finance_domain(document_type: str = None, text: str = None) -> bool:
    """Detect if a document belongs to insurance/finance domain."""
    if document_type and document_type.lower() in (
        'insurance', 'policy', 'travel', 'financial', 'finance', 'contract'
    ):
        return True
    if text:
        text_lower = text[:5000].lower()  # Check first 5000 chars
        matches = sum(1 for term in INSURANCE_FINANCE_TERMS if term in text_lower)
        return matches >= 3  # At least 3 domain terms found
    return False

def get_domain_boost_terms(query: str) -> list:
    """Return matching insurance/finance terms from a query for search boosting."""
    query_lower = query.lower()
    return [term for term in INSURANCE_FINANCE_TERMS if term in query_lower]
...
class VectorService:
    ...
    Includes smart embedding dimension handling.
    ...
    def __init__(
        ...
        # Track dimension issues for analytics
        self.dimension_mismatch_count = 0
        self.last_dimension_check = datetime.now(timezone.utc)
            ...
            for metadata in metadatas:
                if metadata and 'document_hash' in metadata:
                    ...
                    if doc_hash not in registry:
                        registry[doc_hash] = {
                            'document_id': doc_id,
                            'indexed_at': metadata.get('indexed_at', datetime.now(timezone.utc).isoformat()),
                            'chunk_count': 0,
    ...
    async def add_document(
        ...
        try:
            ...
            for i, (_, start_pos, end_pos) in enumerate(chunk_data):
                ...
                # Sanitize metadata values for ChromaDB (only str, int, float, bool, None allowed)
                sanitized_meta = {}
                for k, v in chunk_meta.items():
                    if isinstance(v, (list, dict, tuple, set)):
                        sanitized_meta[k] = json.dumps(v) if v else ""
                    elif v is None:
                        sanitized_meta[k] = ""
                        ...
                        sanitized_meta[k] = v
                chunk_metadatas.append(sanitized_meta)
        ...
        try:
            ...
            if filter_metadata:
                ...
                if not check_results['ids']:
                    self.logger.warning(f"No documents found with filter: {filter_metadata}")
                    return []
    ...
    def get_analytics(self) -> Dict[str, Any]:
        """Get service analytics for monitoring and debugging."""
        return {
